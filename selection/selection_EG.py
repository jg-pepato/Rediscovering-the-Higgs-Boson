...
def select_electrons(tree):
    ...
    ele_dxyErr[ele_dxyErr == 0] = 1e-6
    ele_dzErr[ele_dzErr == 0] = 1e-6
    ...
    if ELECTRON_CUTS["id"] == "WP80":
        ...
        ele_id = np.array(tree.Electron_cutBased) >= 3
    else:
        ele_id = np.ones_like(ele_pt, dtype=bool)
...
def find_best_4lepton_candidate(tree, good_mu, good_e):
        ...
        for idxs in combinations(good_mu, 4):
            ...
            for p1, p2 in [((0,1), (2,3)), ((0,2), (1,3)), ((0,3), (1,2))]:
                if (q[p1[0]] + q[p1[1]] == 0) and (q[p2[0]] + q[p2[1]] == 0):
                    candidates.append({'p4s': [p4s[p1[0]], p4s[p1[1]], p4s[p2[0]], p4s[p2[1]]], 'm_pairs': ((p4s[p1[0]]+p4s[p1[1]]).M(), (p4s[p2[0]]+p4s[p2[1]]).M())})
    ...
    if len(good_e) >= 4:
        for idxs in combinations(good_e, 4):
            ...
            if sum(q) != 0: continue
            p4s = [get_4vector(tree.Electron_pt[j], tree.Electron_eta[j], tree.Electron_phi[j], get_lepton_mass("electron")) for j in idxs]
            for p1, p2 in [((0,1), (2,3)), ((0,2), (1,3)), ((0,3), (1,2))]:
                if (q[p1[0]] + q[p1[1]] == 0) and (q[p2[0]] + q[p2[1]] == 0):
                    candidates.append({'p4s': [p4s[p1[0]], p4s[p1[1]], p4s[p2[0]], p4s[p2[1]]], 'm_pairs': ((p4s[p1[0]]+p4s[p1[1]]).M(), (p4s[p2[0]]+p4s[p2[1]]).M())})
...
# ===== MAIN ANALYSIS =====
def analyze_and_plot(input_files, output_filename="higgs_mass_combined.root"):
