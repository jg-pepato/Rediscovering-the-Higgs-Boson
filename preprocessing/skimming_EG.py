...
def skim_file_doubleeg(input_file, out_file_index, output_dir):
    """
    Skim DoubleElectron events for H -> ZZ -> 4l analysis.
    Requirements: 
    - At least 4 electrons OR (at least 2 electrons AND at least 2 muons)
    ...
    out_name = os.path.join(output_dir, f"skimmedDoubleEG_{out_file_index}.root")
    ...
    for i in range(nentries):
        ...
        # 4ele Channel
        # Needs at least 4 electrons total and at least two of each charge
        if (n_good_ele >= 4) and (n_pos_ele >= 2 and n_neg_ele >= 2):
            pass_skim = True

        # 2ele2Mu Channel
        # Needs at least 2 electrons (1 pos, 1 neg) and 2 muons (1 pos, 1 neg)
